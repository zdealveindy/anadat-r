plot.distmat.links <- function(Dist.mat, thresh=0.05, xlim=NULL, ylim=NULL, verbose=FALSE)
#
...
if(is.null(rownames(Dist.mat))) {
   names <- paste("Var",1:nrow(Dist.mat),sep="")
   rownames(Dist.mat) <- names
   } else {
   names <- rownames(Dist.mat)
   }
X = cmdscale(as.dist(Dist.mat))
par(mai = c(1.0, 1.0, 1.0, 0.5))
plot(X, type="p", xlim=xlim, ylim=ylim, asp=1, xlab="PCoA axis 1", ylab="PCoA axis 2")
text(X, labels=rownames(X), pos=2)
title(main = c("P-value threshold",thresh), family="serif")
n = nrow(Dist.mat)
for(j in 2:n) {
   for(jj in 1:(j-1)) {
      if(Dist.mat[j,jj] <= thresh) {
         lines(c(X[j,1], X[jj,1]), c(X[j,2], X[jj,2]))
         if(verbose) cat("Link between",names[jj],"and",names[j],'\n')
         }
      }
   }
}
